...
const (
	Redis_UserCaptchaID_CahcePrekey  = "EMoney:RoboAdvisor:UserHome_Captcha:"
	Redis_UserCaptchaID_CacheSeconds = 60 * 5
)
...
func (s *redisStore) Set(id string, digits []byte) {
	cacheKey := Redis_UserCaptchaID_CahcePrekey + id
	err := s.redisCache.Set(cacheKey, digits, Redis_UserCaptchaID_CacheSeconds)
	_ = err
}

func (s *redisStore) Get(id string, clear bool) (digits []byte) {
	cacheKey := Redis_UserCaptchaID_CahcePrekey + id
	val, _ := s.redisCache.Get(cacheKey)

	if val != nil {
		digits = []byte(val.([]uint8))
	}
	return
}
...
func NewMemoryStore(collectNum int, expiration time.Duration) Store {
	...
	//s.redisCache = cache.GetRedisCachePoolConf(Reids_UserCaptchaID_ServerUrl, cache.RedisConnPool_MaxIdle, cache.RedisConnPool_MaxActive)

	return s
}

func NewRedisStore(redisConfig *config.RedisInfo, expiration time.Duration) Store {
	s := new(redisStore)
	s.expiration = expiration
	s.redisCache = _cache.GetRedisCacheProvider(redisConfig)
	return s
}

func (s *memoryStore) Set(id string, digits []byte) {
	s.CaptchaSet(id, digits)
	return
	s.Lock()
...
func (s *memoryStore) Get(id string, clear bool) (digits []byte) {
	...
	if clear {
		...
		// XXXx(dchest) Index (s.idByTime) will be cleaned when
...
func (s *memoryStore) CaptchaSet(id string, digits []byte) {
	cacheKey := Redis_UserCaptchaID_CahcePrekey + id
	err := s.redisCache.Set(cacheKey, digits, Redis_UserCaptchaID_CacheSeconds)
	_ = err
}

func (s *memoryStore) CaptchaGet(id string, clear bool) (digits []byte) {
	cacheKey := Redis_UserCaptchaID_CahcePrekey + id
	val, _ := s.redisCache.Get(cacheKey)
