...
type TrainSSOMiddleware struct {
...
func (middleware *TrainSSOMiddleware) Handle(ctx dotweb.Context) error {
	ssoUrl, ssoHash := getSSOUrl(ctx)
	user := contract_train.TrainUserInfo(ctx)

	//没有SSO直接返回未授权
	if user == nil && (len(ssoUrl) == 0 || len(ssoHash) == 0) {
	...
	user.PID = pid
	//用户登录
	contract_train.SetTrainUserInfo(ctx, user)
...
func NewTrainSSOMiddleware() *TrainSSOMiddleware {
	return &TrainSSOMiddleware{}
}
