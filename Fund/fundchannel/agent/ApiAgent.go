...
const (
	...
	//请求缓存时间（秒）
	RequestCacheSeconds = 10 * 60
	//响应缓存时间（秒）
	ResponseCacheSeconds = 10 * 60
	//ApiAgent请求头标识
...
//requestCached 是否缓存请求 如果为true，会先检查本地是否有缓存 有则直接返回缓存数据
//responseCached 是否缓存响应结果 如果为true，如果api服务器检测到服务端的响应结果与api调用客户端自己缓存的数据一致，则不会在响应中包含具体的数据
func get(url string, requestCached bool, responseCached bool) (response *contract.ApiResponse, err error) {

	//发起请求前检查缓存是否存在，如果存在并且没有过期则不请求接口
	cacheData := contract.CacheData{}
	requestKey := ""
	if requestCached || responseCached {
		requestKey = contract.GetRequestHash(url, "")
		obj, exist := getCache(requestKey)
		//类型转换是否成功 如果类型转换失败 则请求接口 并清除本地缓存
		parseSuccess := false
		if exist {
	...
	if errReturn != nil {
		apiAgentLogger.ErrorFormat(errReturn, "【API接口请求异常】URL=【%s】", url)
		return nil, errReturn
	}
	_ = contentType
	_ = intervalTime
...
// Get 发起Get请求，默认启用responseCached缓存，和RequestCacheSeconds秒的requestCached缓存
// url 请求完整地址
// response 响应结果
func Get(url string) (response *contract.ApiResponse, err error) {
	response, err = get(url, true, true)
	if err != nil {
		apiAgentLogger.ErrorFormat(err, "【GetWithResponseCache】【API调用】【异常】 URL=%s  Response=%s", url, _json.GetJsonString(response))
	} else {
		apiAgentLogger.DebugFormat("【GetWithResponseCache】【API调用】【正常】URL=%s  Response=%s", url, _json.GetJsonString(response))
	}
	return response, err
}
